...
public class QuestionsManager {
    ...
    // Refresh all fonts when custom font is toggled
    ...
    private void onDeleteQuestion(int index) {
        showConfirmDialog("Confirm Delete", "Delete this question?", () -> {
    ...
    private void onSave() {
        ...
        JLabel title = new JLabel("Questions Saved", SwingConstants.CENTER);
        title.setForeground(new Color(100, 255, 150));
        title.setFont(new Font("Tahoma", Font.BOLD, 20));
        
        // Try to load checkmark icon - TODO: Need checkmark/success icon
        java.awt.image.BufferedImage icon = model.ResourceLoader.loadImage("/resources/correct.png");
        if (icon != null) {
        ...
        JLabel msg = new JLabel("<html>All questions have been successfully saved to CSV.</html>", SwingConstants.CENTER);
        ...
        okBtn.addActionListener(e -> {
            try {
                ...
                if (!(ans.equals("A") || ans.equals("B") || ans.equals("C") || ans.equals("D"))) {
                    showStyledMessageDialog(dialog, "Invalid Answer", "Correct answer must be A, B, C, or D.", "error");
                    return;
                }

                if (diff < 1 || diff > 4) {
                    showStyledMessageDialog(dialog, "Invalid Difficulty", "Difficulty must be between 1 and 4.", "error");
                    return;
                }
                ...
                showStyledMessageDialog(dialog, "Input Error", "ID and Difficulty must be numbers.", "error");
            }
        });
    ...
    // Styled message dialog using FontManager
    private void showStyledMessageDialog(JDialog parent, String title, String message, String type) {
    ...
    // Styled confirm dialog using FontManager
    private void showConfirmDialog(String title, String message, Runnable onYes) {
        ...
        yesBtn.setFont(new Font("Tahoma", Font.BOLD, 13));
        yesBtn.setBackground(new Color(200, 50, 50));
        yesBtn.setForeground(Color.WHITE);
        yesBtn.setFocusPainted(false);
        yesBtn.setBorder(BorderFactory.createLineBorder(new Color(255, 100, 100), 2));
        ...
        noBtn.setFont(new Font("Tahoma", Font.BOLD, 13));
        noBtn.setBackground(new Color(0, 150, 120));
        noBtn.setForeground(Color.WHITE);
        noBtn.setFocusPainted(false);
        noBtn.setBorder(BorderFactory.createLineBorder(new Color(0, 200, 170), 2));
