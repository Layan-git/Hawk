...
public class SysData {
    ...
    static {
        ...
        loadQuestionsOnStartup();
    }
    
    /**
     * Load questions from either stream (JAR) or file (IDE)
     */
    private static void loadQuestionsOnStartup() {
        // Load from classpath (works in both JAR and IDE with proper classpath setup)
        InputStream csvStream = ResourceLoader.getResourceAsStream("/csvFiles/Questions.csv");
        if (csvStream != null) {
            loadQuestionsFromStream(csvStream);
        } else {
            System.err.println("ERROR: Could not load Questions.csv from classpath. Ensure it's in csvFiles/ folder.");
        }
    }
    ...
    public static void loadQuestions() {
        ...
        if (CSV_PATH == null || CSV_PATH.isEmpty()) {
            System.err.println("CSV_PATH is null or empty");
            return;
        }
        
        File csvFile = new File(CSV_PATH);
        if (!csvFile.exists()) {
            System.err.println("Questions CSV file not found at: " + CSV_PATH);
            return;
        }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(CSV_PATH))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                // Skip header lines - check if it starts with "ID," or contains "Correct"
                if (line.startsWith("ID,") || line.contains("Correct Answer")) {
                    continue;
                }
 ...
 // Simple CSV parser: handles quoted fields with commas and unquoted fields
    ...
    // Works in both JAR and IDE environments
    public static void reloadQuestionsFromCSV() {
        ...
        if (csvStream != null) {
            ...
            // Fallback to file-based loading for IDE development
            loadQuestions();
        }
    }
